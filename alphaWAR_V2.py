...
def main(): 
    ...
    board1 = BrainFlowBoardSetup(board_id= player_1_board_id,
                                 name='Player 1',
                                 serial_port= player_1_serial_port
                                 )
    
    board2 = BrainFlowBoardSetup(board_id= player_2_board_id,
                                 name='Player 2',
                                 serial_port= player_2_serial_port
                                 )
    ...
    while not quit_game:
        ...
        while running:
            ...
            if data1.size and data2.size: 
                ...
                # Display alpha power levels as text
                alpha_text1 = alpha_font.render(f'{board1.get_board_name()} Alpha Power: {alpha_power1:.2f}', True, (255, 0, 0))
                alpha_text2 = alpha_font.render(f'{board2.get_board_name()} Alpha Power: {alpha_power2:.2f}', True, (0, 0, 255))
                screen.blit(alpha_text1, ((width // 2) - center_offset - bar_max_width, 70))
                screen.blit(alpha_text2, ((width // 2) + center_offset, 70))

                pygame.display.flip()
                
                # # Determine background color based on which player has higher alpha power
                # background_color = (255, 200, 200) if alpha_power1 > alpha_power2 else (200, 200, 255)
                ...
                # # Draw the bar graph at the top
                # max_alpha_power = max(alpha_power1, alpha_power2, 1)  # Prevent division by zero
                # bar_width1 = int((alpha_power1 / max_alpha_power) * 300)
                # bar_width2 = int((alpha_power2 / max_alpha_power) * 300)

                # pygame.draw.rect(screen, (255, 0, 0), (50, 50, bar_width1, 30))
                # pygame.draw.rect(screen, (0, 0, 255), (width - 350, 50, bar_width2, 30))
                ...
                # # Display alpha power levels at the top of the screen
                # alpha_text1 = alpha_font.render(f'{board1.get_board_name()} Alpha Power: {alpha_power1:.2f}', True, (255, 0, 0))
                # alpha_text2 = alpha_font.render(f'{board2.get_board_name()} Alpha Power: {alpha_power2:.2f}', True, (0, 0, 255))
                # screen.blit(alpha_text1, (50, 20))
                # screen.blit(alpha_text2, (width - 350, 20))
